...
public class Gregory {
	...
	public Polynomial solve(Integer[] seq) {
		...
		for (int i = 0; i < c.length; i++) {
			p = p.plus(new Polynomial(c[i]).times(factorialPower(i)));
		}
	...
	protected Integer[] calcCoefficients(Integer[] seq) {
		...
		for (int i = 0; containsNonZeroes(s); i++) {
			c.add(s[0]/factorial(i));
			s = delta(s);
		}
		return c.toArray(new Integer[]{});
	}

	protected Integer factorial(Integer n) {
		if (n < 2) return 1;
		int f = 1;
		for (int i = 2; i <= n; i++) {
			f *= i;
	...
	protected Polynomial factorialPower(Integer n) {
